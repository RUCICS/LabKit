...
require_file "$DEPLOY_DIR/migrate.Dockerfile"
...
require_grep 'dockerfile: deploy/migrate.Dockerfile' "$DEPLOY_DIR/docker-compose.yml"
...
require_grep 'entrypoint: \["/bin/sh", "/migrate.sh"\]' "$DEPLOY_DIR/docker-compose.yml"
...
require_fixed 'exec labkit-migrate up' "$DEPLOY_DIR/migrate.sh"
require_fixed 'COPY --from=build /out/labkit-migrate /usr/local/bin/labkit-migrate' "$DEPLOY_DIR/migrate.Dockerfile"
